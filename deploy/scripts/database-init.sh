...
psql -d "postgresql://$ADMINUSER:$ADMIN_URL_PASSWORD@$SERVER/postgres" -c "CREATE USER $STRAPIUSER WITH PASSWORD '$SQL_ESCAPED_PASSWORD';"
...
psql -d "postgresql://$STRAPIUSER:$STRAPI_URL_PASSWORD@$SERVER/$STRAPIDATABASENAME" -c "CREATE SCHEMA $STRAPIUSER AUTHORIZATION $STRAPIUSER;"
